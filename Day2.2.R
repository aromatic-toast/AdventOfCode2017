...
m <- matrix(my.dat, ncol = 16, byrow = TRUE)
...
for(i in 1:nrow(m)){
      print(i)
      row <- m[i,]  #subset rows of matrix one by one
      print(row)
      for(j in 1:length(row)){
            print(j)
            not.j <- row[-j]  #new vector w/ values in row except index j                           
            print(not.j)
            for(k in 1:length(not.j)){
                  div <- row[j]%%not.j[k] #compare j to all others in row
                  if(div==0){ #if no remaineder, then store in div.collect
                        div.collect <- c(div.collect, row[j]/not.j[k])
                  }
            }
      }   
} 
